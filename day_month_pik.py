...
def normalize_category(s: Optional[str]) -> Optional[str]:
    if not s:
        return None
    s = s.strip().lower()
    if "전시" in s or "exhibition" in s:
        return "전시"
    if "공연" in s or "performance" in s or "콘서트" in s or "연극" in s:
        return "공연"
    if "축제" in s or "festival" in s or "페스티벌" in s:
        return "축제"
    return s
...
def fetch_pty_from_kma(lat: float, lon: float, for_date: date) -> Optional[int]:
    ...
    try:
        ...
        def _dist(t: str) -> int:
            try:
                return abs(int(t) - int(target_str))
            except Exception:
                return 9999
...
def weather_suitability(pty: Optional[int], indoor_score: float, outdoor_score: float) -> float:
    if pty is None:
        return 0.0
    if pty == 0:
        return max(indoor_score, outdoor_score)
    return indoor_score

def spotlight_weights(mode: str, weather_active: bool) -> Dict[str, float]:
    if mode == "DAILY":
        return dict(time=0.25, weather=(0.15 if weather_active else 0.0), distance=0.20, interest=0.15, popularity=0.15, rating=0.10)
    return dict(time=0.20, weather=(0.05 if weather_active else 0.0), distance=0.15, interest=0.20, popularity=0.25, rating=0.15)

def in_window(p: Dict[str, Any], mode: str, today: date, month_first: date, month_last: date) -> bool:
    n = norm_post(p)
    sd = parse_date(n["start_date"])
    ed = parse_date(n["end_date"])
...
def score_post_spotlight(
    ...
    if center and n["gps_y"] is not None and n["gps_x"] is not None:
        lat = to_float(n["gps_y"])
        lon = to_float(n["gps_x"])
...
def pick_ids_json(
    ...
    if pty is None and auto_weather and center and "lat" in center and "lon" in center:
        try:
            ...
            pty = None
    weights = spotlight_weights(m, weather_active=(pty is not None))
    interests = interest_categories or []
    rk = radius_km or 10.0
    scored: List[Tuple[float, Any]] = []
    for p in posts:
        s = score_post_spotlight(p, m, center, rk, interests, pty, weights, today)
        if s:
            score, pid = s
            if pid is not None:
